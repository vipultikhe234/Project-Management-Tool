      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-200 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 flex items-center gap-2.5">
            <Settings className="w-8 h-8 text-indigo-600" /> Project Settings
          </h1>
          <p className="text-slate-500 text-sm mt-1 font-medium">
            Configure allowed issue types and manage active Epics for your project.
          </p>
        </div>

        {/* Project Selector */}
        <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-2 shrink-0 w-full md:w-auto shadow-sm">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Project:</span>
          {loadingProjects ? (
            <Loader2 className="w-4 h-4 text-indigo-600 animate-spin" />
          ) : projects.length === 0 ? (
            <span className="text-xs text-rose-600 font-semibold">No active projects</span>
          ) : (
            <select
            ...
            >
              {projects.map(p => (
                <option key={p.uuid} value={p.uuid} className="bg-white text-slate-800">
                  {p.name} ({p.key})
                </option>
              ))}
            </select>
