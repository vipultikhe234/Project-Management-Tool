...
export const CreateSprintModal: React.FC<CreateSprintModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setEndDate('');
      setGoal('');
      setProjectUuid(activeProjectUuid === 'all' ? (projects[0]?.uuid || '') : activeProjectUuid);
      setError('');
    }
  }, [isOpen, defaultSprintName, activeProjectUuid, projects]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const targetProject = projects.find((p) => p.uuid === projectUuid);
    const boardUuid = targetProject?.boards?.[0]?.uuid;
    if (!boardUuid) {
      setError('The selected project does not have an active board.');
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to create sprint. Please check parameters.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold text-slate-400">Plan your team's upcoming iteration cycles</p>
            </div>
          </div>
          <button 
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          {/* Project selection (only shown if All Projects option is selected) */}
          {activeProjectUuid === 'all' && (
